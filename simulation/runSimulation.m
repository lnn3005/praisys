function [time,finish_in_time,functionality] = runSimulation(sizes,crews)

% INPUT

[nodes,links] = createLattice(sizes);
...
total_up_time = sum(sum(v(1:t,:)));
functionality = total_up_time/(numEdges*t);

%fprintf('Finish in time? %s \nTime = %d \nTotal uptime = %d \n',finish_in_time, time, total_up_time);
