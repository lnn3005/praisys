...
funcString = [EdgeSet';init_acc'];
funcString = funcString(1:numEdges*3);

numPeriods = floor(sqrt(sum(workload)));

numCrews = ceil(sum(workload)/numPeriods);
...
for i=1:numEdges
    nameString = ['F[',num2str(EdgeSet(i,1)),',',num2str(EdgeSet(i,2)),']'];
    writeAMPL(fileID,11,nameString,EdgeSet(edge(i).neighbor,:));
end
